...
import lead from "../models/lead.js";
import user from "../models/user.js";
import { calculateCommissionBreakdown } from "../utils/commissionCalculator.js"; // Adjust the path if your helper is saved elsewhere

// --- 1. UNIFIED COMMISSION PIPELINE (STREMLINED) ---
const buildCommissionPipeline = ({ email, monthKey }) => {
  const baseMatch = {};
  if (email) baseMatch.assignTo = email;
...
export const getCommissionList = async (req, res) => {
  try {
    ...
    // 1) Compute current raw sales rows from aggregation
    const computed = await lead.aggregate(
      buildCommissionPipeline({ email, monthKey }),
    );

    // Map and apply target and commission calculations via the JavaScript helper
    const processed = computed.map(row => {
      const breakdown = calculateCommissionBreakdown(row.totalSales);
      
      const monthDate = new Date(row.monthKey + "-01");
      const monthLabel = monthDate.toLocaleDateString("en-US", { month: "short", year: "numeric" }); // e.g., "Jul 2026"
      ...
      return {
        ...
        monthLabel,
        totalSales: row.totalSales,
        basePrice: 0, // Legacy compatibility
        targetCompletionRate: breakdown.targetCompletionRate,
        commissionDue: breakdown.totalCommission,
      ...
      await commissionSnapshot.findOneAndUpdate(
        filter,
        {
          $set: {
            ...
            calcVersion: "v3", // Bumped version for simple sales model
          },
        },
        { upsert: true, new: true },
      );
...
export const getAllAgentsMonthlyPayments = async (req, res) => {
  try {
    ...
    const computed = await lead.aggregate(buildCommissionPipeline({ email }));

    const data = computed.map(row => {
      const breakdown = calculateCommissionBreakdown(row.totalSales);
...
export const updatePaymentInfo = async (req, res) => {
  try {
    ...
    const oldInfo = userDoc.paymentInfo || {};
...
export const getPaymentInfo = async (req, res) => {
  try {
    ...
    return res.json(userDoc.paymentInfo || {});
